...
const computeCeil = (number) => {
  if (number <= 0) throw new Error('ERROR::Number is less than or equal to zero..!')
...
const computePower = (number, powerOf) => {
  ...
  if (number <= 0 || powerOf <= 0) throw new Error('ERROR::Number or power is less than or equal to zero')
